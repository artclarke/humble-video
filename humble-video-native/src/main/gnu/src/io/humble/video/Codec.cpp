...
Codec::getType() {
  ...
  if (mCodec) retval = (MediaDescriptor::Type) mCodec->type;
  return retval;
}
...
Codec::make(const AVCodec *aCodec) {
  ...
  if (aCodec) {
    retval = Codec::make();
    if (retval) retval->mCodec = aCodec;
  }
  return retval;
}
...
Codec::findEncodingCodecByIntID(const int id) {
  ...
  Global::unlock();
  if (codec) retval = Codec::make(codec);
...
Codec::getNumSupportedVideoFrameRates() {
  int count = 0;
  for (const AVRational* p = mCodec->supported_framerates;
      p && !(!p->den && !p->num); p++)
    ++count;
  return count;
}
...
Codec::getSupportedVideoFrameRate(int32_t index) {
  ...
  for (const AVRational* p = mCodec->supported_framerates;
      p && !(!p->den && !p->num); p++, i++)
    if (index == i) return Rational::make(p->num, p->den);
  return 0;
}
...
Codec::getNumSupportedVideoPixelFormats() {
  if (!mCodec) return 0;
  int count = 0;
  for (const enum AVPixelFormat* p = mCodec->pix_fmts; p && (*p != -1); p++)
    ++count;
  return count;
...
Codec::getSupportedVideoPixelFormat(int32_t index) {
  int i = 0;
  for (const enum AVPixelFormat* p = mCodec->pix_fmts; p && (*p != -1);
      p++, i++)
    if (index == i) return (PixelFormat::Type) *p;
...
Codec::getNumSupportedAudioSampleRates() {
  int i = 0;
  for (const int *p = mCodec->supported_samplerates; p && *p; ++p, ++i)
    ;
  return i;
}
int32_t
...
Codec::getNumSupportedAudioFormats() {
  int i = 0;
  for (const enum AVSampleFormat* p = mCodec->sample_fmts; p && (*p != -1);
      p++, i++)
    ;
  return i;
}
...
Codec::getSupportedAudioFormat(int32_t index) {
  ...
  for (const enum AVSampleFormat* p = mCodec->sample_fmts;
      p && (*p != AV_SAMPLE_FMT_NONE); p++, i++)
    if (index == i) return (AudioFormat::Type) *p;
...
Codec::getNumSupportedAudioChannelLayouts() {
  int i = 0;
  for (const uint64_t* p = mCodec->channel_layouts; p && *p; p++, i++)
    ;
  return i;
}
AudioChannel::Layout
Codec::getSupportedAudioChannelLayout(int32_t index) {
  int i = 0;
  for (const uint64_t* p = mCodec->channel_layouts; p && *p; p++, i++)
    if (index == i) return (AudioChannel::Layout) *p;
...
Codec::getNumSupportedProfiles() {
  ...
  for (const AVProfile* p = mCodec->profiles;
      p && p->profile != FF_PROFILE_UNKNOWN; p++, i++)
    ;
  return i;
}
CodecProfile*
Codec::getSupportedProfile(int32_t index) {
  ...
  for (const AVProfile* p = mCodec->profiles;
      p && p->profile != FF_PROFILE_UNKNOWN; p++, i++)
    if (index == i) return CodecProfile::make(p);
  return 0;
}
...
Codec::getNumInstalledCodecs() {
  ...
  void * iterator=0;
  while ((codec = av_codec_iterate(&iterator)) != 0)
    ++numInstalledCodecs;
...
Codec::getInstalledCodec(int32_t index) {
  ...
  const AVCodec * codec = 0;
  void * iterator = 0;
  for (int32_t i = 0; (codec = av_codec_iterate(&iterator)) != 0; i++) {
    if (i == index) return Codec::make(codec);
  }
  return 0;
}
