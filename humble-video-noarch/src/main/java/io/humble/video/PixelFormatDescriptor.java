 ...
 */
  ...
  /**
   * The pixel format is "pseudo-paletted". This means that it contains a<br>
   * fixed palette in the 2nd plane but the palette is fixed/constant for each<br>
   * PIX_FMT. This allows interpreting the data as if it was PAL8, which can<br>
   * in some cases be simpler. Or the data can be interpreted purely based on<br>
   * the pixel format without using the palette.<br>
   * An example of a pseudo-paletted format is AV_PIX_FMT_GRAY8<br>
   * <br>
   * @deprecated This flag is deprecated, and will be removed. When it is removed,<br>
   * the extra palette allocation in AVFrame.data[1] is removed as well. Only<br>
   * actual paletted formats (as indicated by AV_PIX_FMT_FLAG_PAL) will have a<br>
   * palette. Starting with FFmpeg versions which have this flag deprecated, the<br>
   * extra "pseudo" palette is already ignored, and API users are not required to<br>
   * allocate a palette for AV_PIX_FMT_FLAG_PSEUDOPAL formats (it was required<br>
   * before the deprecation, though).
   */
    PIX_FMT_FLAG_PSEUDOPAL(VideoJNI.PixelFormatDescriptor_PIX_FMT_FLAG_PSEUDOPAL_get()),
  /**
   * The pixel format has an alpha channel. This is set on all formats that<br>
   * support alpha in some way, including AV_PIX_FMT_PAL8. The alpha is always<br>
   * straight, never pre-multiplied.<br>
   * <br>
   * If a codec or a filter does not support alpha, it should set all alpha to<br>
   * opaque, or use the equivalent pixel formats without alpha component, e.g.<br>
   ...
   */
    PIX_FMT_FLAG_BAYER(VideoJNI.PixelFormatDescriptor_PIX_FMT_FLAG_BAYER_get()),
  /**
   * The pixel format contains IEEE-754 floating point values. Precision (double,<br>
   * single, or half) should be determined by the pixel size (64, 32, or 16 bits).
   */
    PIX_FMT_FLAG_FLOAT(VideoJNI.PixelFormatDescriptor_PIX_FMT_FLAG_FLOAT_get()),
  ;
