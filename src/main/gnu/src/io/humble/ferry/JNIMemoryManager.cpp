...
VSJNI_MemoryManagerInit(JavaVM* aJVM)
{
  ...
  try
  {
    ...
    cls = env->FindClass("io/humble/ferry/JNIMemoryAllocator");
    if (!cls)
      throw std::runtime_error(
          "could not find io.humble.ferry.JNIMemoryAllocatorclass");
    ...
    if (!sJNIMemoryAllocatorMallocMethod)
      throw std::runtime_error(
          "could not find malloc(int) method in io.humble.ferry.JNIMemoryAllocator");
...
Java_io_humble_ferry_JNIMemoryAllocator_setAllocator(JNIEnv *, jclass,
    jlong aNativeObj, jobject aMemMgr)
{
  // assume that the native object is a RefCounted
  io::humble::ferry::RefCounted *obj =
      *(io::humble::ferry::RefCounted**) &aNativeObj;
  obj->setJavaAllocator(aMemMgr);
}

VS_API_EXPORT jobject JNICALL
Java_io_humble_ferry_JNIMemoryAllocator_getAllocator(JNIEnv *env, jclass,
    jlong aNativeObj)
{
  // assume that the native object is a RefCounted
  io::humble::ferry::RefCounted *obj =
...
Java_io_humble_ferry_FerryJNI_getMemoryModel(JNIEnv *, jclass)
{
  return (jint) sVSJNI_IsMirroringNativeMemoryInJVM;
}

VS_API_EXPORT void JNICALL
Java_io_humble_ferry_FerryJNI_setMemoryModel(JNIEnv *, jclass, jint value)
{
#ifdef VSJNI_USE_JVM_FOR_MEMMANAGEMENT
